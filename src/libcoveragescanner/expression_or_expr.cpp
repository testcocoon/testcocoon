...
{
  clear();
  write(INSTRUMENTATION_OR_EXPR, f,options,s_line_absolute, s_line_org, s_column_org, s_line_pre, s_column_pre, e_line_absolute, e_line_org, e_column_org, e_line_pre, e_column_pre,depth);
  if (expression_id1)
...
ExpressionOrExpr::~ExpressionOrExpr()
{
  FUNCTION_TRACE;
}

#if LOG
void ExpressionOrExpr::PDEBUG() const
{
  FUNCTION_TRACE;
  DEBUG1("ExpressionOrExpr");
  Expression::PDEBUG();
}
#endif
