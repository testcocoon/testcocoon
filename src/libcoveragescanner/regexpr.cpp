...
#define FAIL2(m, code)      { re_report(m); upd_err_code(code,errp); return NULL; }
...
static void upd_err_code(int code,int *errp)
{
   if(errp) 
      *errp = code; 
}
...
int re_comp_w(regexp** rpp, const CHAR_TYPE* exp)
{
        ...
        FAIL("Invalid out regexp pointer", REGEXP_BADARG);
    {
        ...
        /* Allocate space. */
        r = (regexp *)MALLOC(sizeof(regexp) + (size_t)co.regsize*sizeof(CHAR_TYPE));
        if (r == NULL)
...
static int regmatch_(struct exec* ep, CHAR_TYPE* prog)
{
	...
	for (scan = prog; scan != NULL; scan = next) {
        ...
        case CCLASS: {
            ...
            {
                if (ep->reginput == LIT('\0') ||
                    !(*class_table_f[(int)(*opnd)])(*ep->reginput))
                    return 0;
            }
            else
            {
...
static size_t regrepeat(struct exec* ep, CHAR_TYPE* node)
{
    ...
    case CCLASS:
        ...
        if(ch >= 0)
        {
            for (scan = ep->reginput; *scan && (*class_table_f[(int)(ch)])(*scan); scan++)
                count++;
        }
        else
        {
