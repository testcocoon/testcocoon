...
ExpressionSubCondition::ExpressionSubCondition(const char *f,instrumentation_option_t options,long s_line_absolute, long s_line_org, long s_column_org, long s_line_pre, long s_column_pre, long e_line_absolute, long e_line_org, long e_column_org, long e_line_pre, long e_column_pre,long depth,Expression *expression_id1)
{
  clear();
  write(INSTRUMENTATION_SUB_CONDITION, f,options,s_line_absolute, s_line_org, s_column_org, s_line_pre, s_column_pre, e_line_absolute, e_line_org, e_column_org, e_line_pre, e_column_pre,depth);
  writePatchArea(0, s_line_absolute, s_column_pre, e_line_absolute, e_column_pre,depth);
  if (!getOption().coverage_condition)
    instrumentation_option.coverage_enable=0;
  if (expression_id1)
	addChildren(expression_id1);
}

ExpressionSubCondition::~ExpressionSubCondition()
{
  FUNCTION_TRACE;
}

std::list<const Patch*> ExpressionSubCondition::generatePatchCode(code_style_t t,const char *table_name,long index)
{
...
#if LOG
void ExpressionSubCondition::PDEBUG() const
{
  FUNCTION_TRACE;
  DEBUG1("ExpressionSubCondition");
  Expression::PDEBUG();
}
#endif

long ExpressionSubCondition::indexes() const
{
  FUNCTION_TRACE;
  return 2;
}

CSMESFile::instrumentation_t ExpressionSubCondition::instrumentationType() const
{
  FUNCTION_TRACE;
