...
#ifndef EXPRESSION_IF_THEN_H
#define EXPRESSION_IF_THEN_H
...
class StringPool;
class Source;
class ExpressionIfThen: public Expression
{
public:
    ExpressionIfThen(const char *f,instrumentation_option_t options,long s_line_absolute, long s_line_org, long s_column_org, long s_line_pre, long s_column_pre, long e_line_absolute, long e_line_org, long e_column_org, long e_line_pre, long e_column_pre,long depth,
...
private:
    void operator=(const ExpressionIfThen &);
    ExpressionIfThen(const ExpressionIfThen &);
};
