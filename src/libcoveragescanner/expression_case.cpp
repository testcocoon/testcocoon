...
ExpressionCase::ExpressionCase(const char *f,instrumentation_option_t options,long s_line_absolute, long s_line_org, long s_column_org, long s_line_pre, long s_column_pre, long e_line_absolute, long e_line_org, long e_column_org, long e_line_pre, long e_column_pre,long depth)
{
  clear();
  write(INSTRUMENTATION_CASE, f,options,s_line_absolute, s_line_org, s_column_org, s_line_pre, s_column_pre, e_line_absolute, e_line_org, e_column_org, e_line_pre, e_column_pre,depth);
...
std::list<const Patch*> ExpressionCase::generatePatchCode(code_style_t t,const char *table_name,long index)
{
...
#if LOG
void ExpressionCase::PDEBUG() const
{
  FUNCTION_TRACE;
  DEBUG1("ExpressionCase");
  Expression::PDEBUG();
}
#endif

long ExpressionCase::indexes() const
{
  FUNCTION_TRACE;
  return 1;
}

CSMESFile::instrumentation_t ExpressionCase::instrumentationType() const
{
  FUNCTION_TRACE;
