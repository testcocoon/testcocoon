...
UndoCmdExecutionInsert::UndoCmdExecutionInsert (CSMes *csmes_p,const ExecutionName &name,QUndoCommand *parent) : QUndoCommand("",parent)
{
  ...
  setText(QTime::currentTime().toString(UNDO_TIME_FORMAT)+QObject::tr("Insert execution '%1'.").arg(name))  ;
}

UndoCmdExecutionInsert::~UndoCmdExecutionInsert ()
{
  if (m_exec)
    delete m_exec;
}

void UndoCmdExecutionInsert::undo()
{
...
void UndoCmdExecutionInsert::redo()
{
  CSMesBackgroundComputations::Pauser statistic_pauser;
  if (m_exec)
