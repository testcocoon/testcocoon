...
\lstset{keywordstyle=\bfseries}
\part{Tutorial} \cutname{tutorial.html} \input{testcocoon_qt_tutorial.tex}
...
{\itshape Syntax:} \lstinline$int __coveragescanner_register_library(const char *library_name)$ \bigskip \\
    ...
    \begin{description}
       \item[0] Success.
       \item[-1] Error: library cannot be loaded.
       \item[-2] Error: invalid library name.
       \item[1] Success, but library was already registered. Use count is incremented.
       \item[2] Success, but library is not instrumented.
    \end{description}


\subsection{\CppLibraryFunction{unregister\_library}} 
{\itshape Syntax:} \lstinline$int __coveragescanner_unregister_library(const char *library_name)$ \bigskip \\
    \verb$__coveragescanner_unregister_library$ cancel a register of a shared library loaded on demand.
    \bigskip \\
    Returned values:
    \begin{description}
       \item[0] Success.
       \item[-1] Error: library not registered.
       \item[-2] Error: invalid library name.
       \item[1] Success, but library is not unregistered because the use count is not null.
       \item[2] Success, but library is not instrumented.
    \end{description}
